...
typedef NS_OPTIONS(NSUInteger, SDWebImageOptions) {
     ...
     */
    SDWebImageProgressiveDownload = 1 << 3, // 图片会变下载变现实
     ...
     */
    SDWebImageRefreshCached = 1 << 4,  // 将硬盘缓存交个系统自带的NSURLCache去处理
     ...
     */
    SDWebImageHandleCookies = 1 << 6, // 通过设置NSMutableURLRequest.HTTPShouldHandleCookies = YES来处理缓存NSHTTPCookieStore中的cookie
...
@property (strong, nonatomic, readonly) SDImageCache *imageCache; //负责管理cache，涉及内存缓存和硬盘保存
@property (strong, nonatomic, readonly) SDWebImageDownloader *imageDownloader; //负责从网络下载图片
